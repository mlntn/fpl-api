...
class League extends LeagueSimple
{
    ...
    {
        $crawler = $this->getDom("http://fantasy.premierleague.com/my-leagues/{$league_id}/standings/" . ($page > 1
            ? "?ls-page={$page}" : ''));
        ...
        foreach ($users as $i => $ui) {
            ...
            $user->id              = (int)preg_replace('~^/entry/(\d+)/.+?$~', '$1',
              $u->filterXPath('//td[3]/a')->attr('href'));
