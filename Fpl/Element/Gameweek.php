...
class Gameweek extends Element
{
    ...
    public function load($gameweek)
    {
        ...
        $crawler = $this->getDom("http://fantasy.premierleague.com/fixtures/{$gameweek}/",
        ...
        foreach ($games as $g) {
            ...
            $home_team_id     = (int)preg_replace('~^.+badge_(\d+).+$~', '$1',
              $gc->filterXpath('//td[3]/img')->attr('src'));
            $away_team_id     = (int)preg_replace('~^.+badge_(\d+).+$~', '$1',
              $gc->filterXpath('//td[5]/img')->attr('src'));
